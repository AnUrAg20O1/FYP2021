...
metrics = {'ep': [], 'avg': [], 'min': [],
           'max': []}  # metrics recorded for graph
...
for run in range(RUNS):
    ...
    while not done:
        ...
        newState, reward, done, _ = env.step(
            action)  # perform action on enviroment
    ...
    if run % UPDATE_EVERY == 0:
        ...
        print("Run:", run, "Average:", averageCnt, "Min:",
              min(latestRuns), "Max:", max(latestRuns))
